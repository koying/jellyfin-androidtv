...
class MediaContentProvider : ContentProvider(), KoinComponent {
	 ...
	 */
	private suspend fun searchItems(query: String, qLimit: Int): ItemsResult? = withContext(Dispatchers.Default) {
		...
		val query = ItemQuery().apply {
			...
			sortBy = arrayOf(ItemSortBy.DateCreated)
		...
		return@withContext apiClient.getItems(query.apply {
			...
			imageTypeLimit = 1
			limit = qLimit
			fields = arrayOf(ItemFields.DateCreated, ItemFields.DateCreated,ItemFields.Studios,ItemFields.Genres,ItemFields.Taglines,ItemFields.ProviderIds,ItemFields.CriticRatingSummary,ItemFields.MediaStreams)
		})
	}
	...
	private fun getSuggestions(query: String, limit: Int): Cursor = runBlocking {
		...
		var yearFormatter = SimpleDateFormat("yyyy")
		searchResult?.items?.forEach { item ->
			val imageUri = Uri.parse(apiClient.GetImageUrl(item, ImageOptions().apply {
			...
			val release = when {
				item.premiereDate != null -> yearFormatter.format(item.premiereDate)
				else -> ""
			}
